...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $customer_id = filter_input(INPUT_POST, 'user_id', FILTER_VALIDATE_INT);
    ...
    if (!$customer_id || !$note_type || !$note_content) {
    ...
    if (!isset($_SESSION['username'])) {
        $stmt = $conn->prepare("SELECT name FROM kunden WHERE id = :user_id");
    ...
    try {
        $stmt = $conn->prepare("INSERT INTO notes_customer (user_id, type, content, created_at, author) 
        ...
        // Loggen des Eintrags
        logAction('INSERT', 'notes_customer', 'user_id: ' . $customer_id . ', type: ' . $note_type);

        $note_id = $conn->lastInsertId();
        $stmt = $conn->prepare("SELECT * FROM notes_customer WHERE id = :id");
